...
export class AdminDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.req.getModels().subscribe(data => {
      this.models = data
    });
  ...
  setForm(selectedModel) {
    if (selectedModel !== undefined) {
      this.isLoading = true;
      this.req.getData(selectedModel).subscribe(data => {
        this.accessoriesForm = this.formBuilder.group({
          model: new FormControl(data['model'], Validators.required),
          halfKitItems: this.formBuilder.array(data['halfKitItems'].map(name => this.formBuilder.group({
            name: new FormControl(name, Validators.required),
          }))),
          fullKitItems: this.formBuilder.array(data['fullKitItems'].map(name => this.formBuilder.group({
            name: new FormControl(name, Validators.required),
          }))),
          ...
          accessories: this.formBuilder.array(data['accessories'].map((data) => this.formBuilder.group({
  ...
  onSubmit() {
    ...
    data['halfKitItems'] = data['halfKitItems'].map((v)=> v["name"]);
    data['fullKitItems'] = data['fullKitItems'].map((v)=> v["name"]);
    this.req.postData(data).subscribe((d) => {
      console.log("siujduiklhsd");
